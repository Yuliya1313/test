...
while i < len(actions):
    ...
    """проверить операции в действии на соответствие операции 1го приоритета
    если она не соответствует - ничего не делаем
    если она соответствует, то:
    *вычисляем результат для числа в этом действии и соседе слева
    *записать результат у соседа справа
...
while i < len(actions):
    ...
    """проверить операции в действии на соответствие операции 2го приоритета
    если она не соответствует - ничего не делаем
    если она соответствует, то:
    *вычисляем результат для числа в этом действии и соседе слева
    *записать результат у соседа слева
    *удалить текущее действие
    """
    action = actions[i]
    operation = action.get('opr')
